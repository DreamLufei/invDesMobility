...
recommand_step_lr = {
    ...
    'gen':{
        ...
        "carbon_24": 1e-5,
        "mp_20": 5e-6,
        "mobility2d_highquality280": 5e-6
    },
}
        ...
        if len(ckpts) > 0:
            ...
            if ckpt is None:
                ...
                for ck in ckpts:
                    if 'last' in ck.parts[-1]:
                        continue
                    parts = ck.parts[-1].split('-')[0].split('=')
                    if len(parts) == 2 and parts[1].isdigit():
                        epoch_ckpts.append((int(parts[1]), ck))
                if epoch_ckpts:
                    ckpt = str(max(epoch_ckpts, key=lambda item: item[0])[1])
                elif len(ckpts) == 1:
                    ckpt = str(ckpts[0])
                else:
                    raise ValueError(f'Unable to choose checkpoint from {model_path}: {ckpts}')
