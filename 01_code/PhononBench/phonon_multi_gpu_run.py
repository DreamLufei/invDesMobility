...
def get_params():
    ...
    parser.add_argument(
        '--imag_threshold',
        type=float,
        default=float(os.environ.get("PHONONBENCH_IMAG_THRESHOLD", "1e-3")),
        help='Treat frequencies below -imag_threshold as imaginary/unstable.'
    )
...
for yml_file in sorted_part_files:
    ...
    try:
        ...
        if np.any(all_freqs < -abs(params.imag_threshold)):
            imag_count += 1
            Lable.append([structure_name,'unStable'])
        else:
            no_imag_count += 1
...
with open(summary_file, 'w') as f:
    ...
    f.write(f"Imaginary-frequency threshold: -{abs(params.imag_threshold)}\n")
